...
class DhumbalEnv:
    ...
    def step(self, action) -> Tuple[np.ndarray, float, bool]:
        ...
        print(f"Turn {new_state.turn_count+1}, Player {player}, Phase {new_state.phase}, Hand size {hand_size}, Action {action}")
        ...
        if new_state.phase == 'call':
            if action and self.game.can_call_jhyap(new_state.hands[player]):
                ...
                new_state.done = True
                print(f"Episode ended: Jhyap call, Winner {new_state.winner}, Reward {reward}")
            ...
            if len(new_state.hands[player]) >= HAND_SIZE:
                ...
                if self.deck:
                    ...
                    print(f"Episode ended: Deck exhausted, Winner {new_state.winner}, Reward {reward}")
            else:
                reward = -100.0
                new_state.done = True
                new_state.winner = (player + 1) % self.game.num_players
            ...
            if not new_state.done:
                ...
                if new_state.turn_count >= MAX_TURNS:
                    ...
                    new_state.winner = hand_values.index(min(hand_values))
                    reward = -sum(min(v, MAX_PAYMENT) for v in hand_values) if new_state.winner != player else sum(min(v, MAX_PAYMENT) for i, v in enumerate(hand_values) if i != player)
                    print(f"Episode ended: Max turns reached, Winner {new_state.winner}, Reward {reward}")
...
class PPO:
    ...
    def train(self, batch_size: int):
        ...
        old_probs = np.array([m[2] for m in minibatch])
        rewards = np.array([m[3] for m in minibatch])
        next_states = np.array([m[4] for m in minibatch], dtype=np.float32)
        dones = np.array([m[5] for m in minibatch])

        with tf.device('/GPU:0'):
            values = self.critic(states, training=False).numpy().flatten()
            next_values = self.critic(next_states, training=False).numpy().flatten()
            advantages = rewards + (1 - dones) * GAMMA * next_values - values

            # Actor update
            with tf.GradientTape() as tape:
                probs = self.actor(states, training=True)
                action_probs = tf.reduce_sum(probs * tf.one_hot(actions, self.max_action_size), axis=1)
            ...
            with tf.GradientTape() as tape:
                ...
                critic_loss = tf.reduce_mean(tf.square(rewards + (1 - dones) * GAMMA * next_values - values))
...
def train_agent(agent: LearningBasedAI, game: DhumbalGame, opponents: List[RuleBasedAI], batch_size: int):
    ...
    csv_path = '/content/ppo_training_results.csv'
    try:
        with open(csv_path, 'w', newline='') as csvfile:
            ...
            for episode in range(TRAINING_EPISODES):
                ...
                while not env.state.done:
                    ...
                    agent.agent.remember(state, action_idx, prob, reward, next_state, done)
                    agent.agent.train(batch_size)
                    episode_reward += reward
                    turns += 1
                    state = next_state
                    if done:
                        break
                # Log episode results after the loop
                print(f"Episode {episode+1} ended after {turns} turns, Reward {episode_reward}")
                ...
                # Save model every 500 episodes or on first episode
                if episode == 0 or (episode + 1) % 500 == 0:
                    agent.agent.actor.save_weights('/content/ppo_actor.weights.h5')
                    agent.agent.critic.save_weights('/content/ppo_critic.weights.h5')
                    print(f"Saved PPO models at episode {episode+1}")
                    if os.path.exists('/content/ppo_actor.weights.h5') and os.path.exists('/content/ppo_critic.weights.h5'):
                        print(f"Confirmed models saved: /content/ppo_actor.weights.h5 and /content/ppo_critic.weights.h5")
                    else:
                ...
                if len(win_rates) >= CONVERGENCE_EPISODES:
                    ...
                    if recent_win_rate > best_win_rate:
                        ...
                        if os.path.exists('/content/ppo_actor.weights.h5') and os.path.exists('/content/ppo_critic.weights.h5'):
                            print(f"Confirmed best models saved: /content/ppo_actor.weights.h5 and /content/ppo_critic.weights.h5")
                if episode % 500 == 0:
                    print(f"Training {agent.name}, Episode {episode+1}/{TRAINING_EPISODES}, Recent Win Rate: {np.mean(win_rates[-500:]) if win_rates else 0:.3f}, Avg Turns: {np.mean(episode_turns[-500:]) if episode_turns else 0:.1f}")
            # Save final model
            if best_model:
            ...
            agent.agent.actor.save_weights('/content/ppo_actor.weights.h5')
            agent.agent.critic.save_weights('/content/ppo_critic.weights.h5')
            print("Final PPO models saved to /content/ppo_actor.weights.h5 and /content/ppo_critic.weights.h5")
            if os.path.exists('/content/ppo_actor.weights.h5') and os.path.exists('/content/ppo_critic.weights.h5'):
                print(f"Confirmed final models saved: /content/ppo_actor.weights.h5 and /content/ppo_critic.weights.h5")
            if os.path.exists(csv_path):
                file_size = os.path.getsize(csv_path)
                print(f"Training results saved to {csv_path}, File size: {file_size} bytes")
                with open(csv_path, 'r') as f:
                    content = f.read()
                    print(f"CSV content preview: {content[:200]}...")  # Preview first 200 chars to verify data
